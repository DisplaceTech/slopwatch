...
check. On a **public** repo, any user could comment "@claude …" and trigger an agent running
with `contents: write` + `bypassPermissions` that checks out the PR head branch
(attacker-controllable on fork PRs) while `CLAUDE_CODE_OAUTH_TOKEN` was in scope — the classic
"pwn request" pattern (untrusted code executed with secrets + write access).

**Root cause:** Workflows triggered by `issue_comment` / `pull_request_review[_comment]` run in
the base-repo context (with secrets) but the comment author is untrusted. Gating only on the
comment *body* (`@claude`) authorizes the world.

**Guardrail:** Add a `getCollaboratorPermissionLevel` check as the FIRST step and gate the
checkout *and* the agent step on `allowed == 'true'` (write/admin only) — so untrusted code is
never even checked out for a non-collaborator. `plan-approval-gate.yml` already does this; the
feedback workflow now mirrors it. Author-association (`OWNER`/`MEMBER`/`COLLABORATOR`) in the
job `if:` is an acceptable lighter-weight alternative. General rule: any comment/review-triggered
job that runs code or an agent must authorize the actor before doing anything with the payload.
